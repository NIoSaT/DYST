...
int main(int argc,char* argv[]){
    ...
    checksecretmsg(secretmsg,bit,chkbit,tolerance,rng,MAXITER,fp,DYST_EXT);

    // Alternative to CLP eval: enumerate desired cases
    /*
    for(int b=14;b<=22;b++){
        checksecretmsg(secretmsg,b,0,0,rng,MAXITER,fp,DYST_BASIC);
        resetrand(rng);

    }
    */

    // completely DYST_EXT: b=14...18 cb=6...10 t=1..5
    /*
    for(int b=18;b<=18;b++) for(int cb=6;cb<=10;cb++) for(int t=1;t<=5;t++){
        checksecretmsg(secretmsg,b,cb,t,rng,MAXITER,fp,DYST_EXT);
        resetrand(rng);
    }
    */
    if(filename != NULL)
