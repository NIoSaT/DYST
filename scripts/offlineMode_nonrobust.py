...
def offlineMode_Basic(packet):
    ...
    if isPktOfInterest(packet):
        counter_interest += 1
        critical_fraction, seconds = math.modf(packet.time)
        if (critical_fraction > crit_fraction_high or critical_fraction < crit_fraction_low):
            res.append("{},{},{},{},{},{}\n".format(False, counter_interest, counter_total, -1, packet.time, -1))
            return


        hash = getHashValue(getInputValues(packet))
        hash = list(map(int,list(BitArray(bytes=hash.digest()).bin[:len(currTarget)])))
        matchPerc = getMatchPercent(hash,list(currTarget))

        if (matchPerc == 1.0):
            if lastHit != 0:
                res.append("{},{},{},{},{},{}\n".format(True, counter_interest, counter_total, matchPerc, packet.time, (packet.time-lastHit)))
            else:
                res.append("{},{},{},{},{},{}\n".format(True, counter_interest, counter_total, matchPerc, packet.time, -1))
            cm_array_current+=1
            ba_curr = getStringToBinary(cm_array[cm_array_current])
            lastHit = packet.time
        else:
            res.append("{},{},{},{},{},{}\n".format(False, counter_interest, counter_total, matchPerc, packet.time, -1))
    ...
    if isPktOfInterest(packet):
        ...
        if (matchCount >= targetCount):
            if checkBitFlips(hash,ba_curr):
                if lastHit != 0:
                    res.append("{},{},{},{},{},{}\n".format(True, counter_interest, counter_total, matchCount, packet.time, (packet.time-lastHit)))
                else:
                    res.append("{},{},{},{},{},{}\n".format(True, counter_interest, counter_total, matchCount, packet.time, -1))
                cm_array_current+=1
                ba_curr = getStringToBinary(cm_array[cm_array_current])
                lastHit = packet.time
            else:
                res.append("{},{},{},{},{},{}\n".format(False, counter_interest, counter_total, matchCount, packet.time,-1))
...
if sys.argv[1] == "--help" or sys.argv[1] == "-h":
    ...
    print("Usage:")
    print("offlineMode_nonrobust.py <Covert Message File> <# of bytes at once> <pcap File> <mode [basic,ext]> <broadcast IP of recording> <output File> [<Match Count>]")
    exit(5)
